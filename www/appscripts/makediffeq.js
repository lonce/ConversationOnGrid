    ...
    function(){
    	...
    	var coef2polystring = function(coefs){
    		var numeqs=coefs.length;
  			var numcoefs;
  			var termcount;
  			var vars3d= ["", "x[0]","x[1]", "x[2]", "x[0]*x[0]", "x[0]*x[1]","x[0]*x[2]","x[1]*x[1]","x[1]*x[2]","x[2]*x[2]"];
    		...
    		for (i=0;i<numeqs;i++){
    			...
    			termcount=0; // prvents the first term in the polynomial from being preceeded by a sign. 
    			for(j=0;j<numcoefs;j++){
                    ...
                    else if (j===0) outstring += "0"; // this is the constant coef === 0 case
                    termcount++;
    			}
